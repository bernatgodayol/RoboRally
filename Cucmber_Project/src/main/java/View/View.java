...
//	}
	...
	public void cardUpdated(CardStatus newCardStatus) {
		...
		for(int i=0; i<newCardStatus.getCards().length; i++) {
			imageView = cardManager.getCardImage(newCardStatus.getCardsNum()[i]);	
			imageView.setFitHeight(40);
			imageView.setFitWidth(40);
			imageView.setOnMouseClicked(new EventHandler<Event>() {
				public void handle(Event event) {
					notifyMenuViewUpdated(this.i, newCardStatus.getPlayer());
				}
			});
//			imageView.setOnMouseClicked(handler);
